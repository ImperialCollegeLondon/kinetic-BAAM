...
switch stepName
    case 'ads' %for the case when the stepName is indicated as the adsorption step ('ads')
        P = parameters.P_ads(t.*timeRef)./PRef; % dimensionless pressure inside the column as the adsorption pressure as a function of time
        % parameters.h_out = 5;
        % parameters.h_in = 5;
        ...
        [q1_star, q2_star] = DSL(P.*PRef, parameters.y1_in, T.*TRef, parameters.qsb_1, parameters.qsd_1, parameters.qsb_2, parameters.qsd_2, parameters.bo_1, parameters.do_1, parameters.bo_2, parameters.do_2, parameters.delUb_1, parameters.delUd_1, parameters.delUb_2, parameters.delUd_2);
    ...
    case 'blo' % for the case when the stepName is indicated as the blowdown step ('blo')
        P = parameters.P_blo(t.*timeRef)./PRef; % dimensionless pressure inside the column as a function of time
    ...
    case 'evac'
        P = parameters.P_evac(t.*timeRef)./PRef; % dimensionless pressure inside the column as a function of time
        % Competitive equilibrium adsorbed amount and LDF coefficient for
        % both species at P, T, y1
        % at OUTLET COMPOSITION (y1) at P,T.
        [q1_star, q2_star] = DSL(P.*PRef, y1, T.*TRef, parameters.qsb_1, parameters.qsd_1, parameters.qsb_2, parameters.qsd_2, parameters.bo_1, parameters.do_1, parameters.bo_2, parameters.do_2, parameters.delUb_1, parameters.delUd_1, parameters.delUb_2, parameters.delUd_2);
    ...
    case 'pres'
        P = parameters.P_press(t.*timeRef)./PRef; % dimensionless pressure inside the column as a function of time
...
if parameters.modelType == "isothermal"
    ...
    %     dTdt = coefft1.*(...
    %         + ((parameters.F_in)./parameters.V_column.*parameters.cp_g.*(373-TRef.*T)) ...
